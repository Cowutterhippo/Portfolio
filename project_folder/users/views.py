		...
		if form.is_valid():
			...
			return redirect(
				'/users/login?message{}{}'.format(
					"Signup Successful, Please Login", next_url))
...
class Login(View):
	def get(self, request):
		...
		request.context_dict['message'] = requestt.GET.get('message', '')
		request.context_ict['form'] = AuthenticationForm()
...
class ChangePassword(View):
	...
	def post(self, request):
		...
		if form.is_valid():
			...
			request.context_dict['form'] = form
			return render(request, 'users/changePassword', request.context_dict)
